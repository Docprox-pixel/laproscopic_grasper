...
Sends joint commands directly via the ros2 action client.
Updated for the KUKA iiwa14 (7-DOF arm) + 5-DOF Grasper configuration.
"""
...
# joint names used by the trajectory action controllers
ARM_JOINTS = [
...
def send_shaft_command(position, duration=1):
    cmd = f"""ros2 action send_goal /shaft_controller/follow_joint_trajectory control_msgs/action/FollowJointTrajectory "{{
  trajectory: {{
    joint_names: ['shaft_translation_joint'],
    points: [{{
      positions: [{position}],
      velocities: [0.0],
      time_from_start: {{sec: {duration}, nanosec: 0}}
...
def home():
    ...
    send_shaft_command(0.0, 2)
    status_var.set("Homing robot...")

def retract_shaft():
    s_shaft.set(0.0)
    send_shaft_command(0.0, 1)
    status_var.set("Retracting trocar shaft...")
...
shaft_card = card(root, "TROCAR MECHANISM")
s_shaft = slider_row(shaft_card, "shaft_sliding (0..0.08)", 0.0, 0.08)
...
tk.Button(quick_frame, text="⏪ RETRACT", command=retract_shaft, bg=YELLOW, fg=DARK, width=15).pack(side="left", expand=True, padx=2)
