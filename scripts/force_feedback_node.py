...
class ForceFeedbackNode(Node):
    ...
    def process_feedback(self):
        # Prefer direct contact sensors for grip force measurement
        if self.jaw_l_contact_force > 0.01 or self.jaw_r_contact_force > 0.01:
            # Use the maximum of the two jaws to ensure we detect pressure reliably
            self.grip_force = max(self.jaw_l_contact_force, self.jaw_r_contact_force)
        else:
            # Fallback to joint effort and shaft force if no direct contact
